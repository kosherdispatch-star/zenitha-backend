...
# ── CACHE HOROSCOPE ──────────────────────────────────────────
# L'horoscope du jour est identique pour tout le monde pour un signe donné.
# On le génère UNE seule fois par jour et par signe, puis on sert la copie.
# Résultat : au maximum 12 générations IA par jour, quel que soit le trafic.
_cache_horoscope = {}  # clé = "AAAA-MM-JJ|signe" → texte

def _cle_horoscope(signe):
    return f"{datetime.utcnow().strftime('%Y-%m-%d')}|{signe.lower().strip()}"
# ─────────────────────────────────────────────────────────────

# Import des modules de connaissance et synthèse Zenitha (IA)
try:
    from zenitha_synthese import generer_portrait, repondre_question, analyser_reve, horoscope_du_jour, calculer_compatibilite
    IA_DISPONIBLE = True
...
def horoscope():
    ...
    try:
        ...
        # Cache : si l'horoscope du jour pour ce signe existe déjà, on le renvoie
        # sans rappeler l'IA. Économie majeure quand beaucoup de visiteurs consultent.
        cle = _cle_horoscope(signe)
        if cle in _cache_horoscope:
            return jsonify({'success': True, 'horoscope': _cache_horoscope[cle], 'cache': True})

        texte = horoscope_du_jour(signe, transits_actuels)
        _cache_horoscope[cle] = texte
        return jsonify({'success': True, 'horoscope': texte, 'cache': False})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 400

@app.route('/compatibilite', methods=['POST'])
def compatibilite():
    """Lecture de compatibilité entre deux thèmes (fonctionnalité premium)"""
    if not IA_DISPONIBLE:
        return jsonify({'success': False, 'error': 'Modules IA non disponibles'}), 503
    try:
        data = request.json
        theme_a = data.get('theme_a', {})
        theme_b = data.get('theme_b', {})
        prenom_a = data.get('prenom_a', 'toi')
        prenom_b = data.get('prenom_b', "l'autre")
        if not theme_a or not theme_b:
            return jsonify({'success': False, 'error': 'Deux thèmes sont requis'}), 400
        texte = calculer_compatibilite(theme_a, theme_b, prenom_a, prenom_b)
        return jsonify({'success': True, 'compatibilite': texte})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 400
