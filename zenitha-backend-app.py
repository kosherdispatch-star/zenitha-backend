...
def calculer_theme(annee, mois, jour, heure, minute, latitude, longitude, timezone_offset=1):
    ...
    # Nœuds lunaires
    noeud, _ = swe.calc_ut(jd, swe.TRUE_NODE)
    noeud_nord = degre_en_signe(noeud[0])
    noeud_sud_deg = (noeud[0] + 180) % 360
    noeud_sud = degre_en_signe(noeud_sud_deg)
...
def phase_lune():
    ...
    try:
        ...
        soleil, _ = swe.calc_ut(jd, swe.SUN)
        lune, _ = swe.calc_ut(jd, swe.MOON)
